...
def upsert_match(cursor, ev, league_name):
    ...
    kickoff, ottawa_date = utc_to_ottawa(ev.get("strTimestamp"))
    # Use Ottawa-converted date; fall back to API dateEvent, converting bad formats
    fallback_date = ev.get("dateEvent", "")
    date_str = ottawa_date if ottawa_date else fix_date(fallback_date, season=season, kickoff_epoch=kickoff)
