...
import { fetchMatchAnalysis, fetchMatchReport, type FullAnalysis, type MatchReport, type FormData, type Pattern } from "@/lib/api";
import {
    ...
    <div className="flex gap-1 flex-wrap">
      {streak.map((r, i) => (
        <span key={i} className={`w-7 h-7 flex items-center justify-center rounded text-xs font-bold
          ${r === "W" ? "bg-green-500/20 text-green-400" : r === "D" ? "bg-yellow-500/20 text-yellow-400" : "bg-red-500/20 text-red-400"}`}>
          {r}
        </span>
      ))}
    </div>
...
function MarkdownReport({ content }: { content: string }) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (trimmed.startsWith("# ") && !trimmed.startsWith("## ")) {
      elements.push(<h1 key={i} className="text-xl font-bold mt-6 mb-2 text-foreground">{trimmed.slice(2)}</h1>);
    } else if (trimmed.startsWith("## ")) {
      elements.push(<h2 key={i} className="text-lg font-bold mt-5 mb-2 text-primary border-b border-border/50 pb-1">{trimmed.slice(3)}</h2>);
    } else if (trimmed.startsWith("### ")) {
      elements.push(<h3 key={i} className="text-sm font-medium text-muted-foreground mt-1 mb-3">{trimmed.slice(4)}</h3>);
    } else if (trimmed.startsWith("- ") || trimmed.startsWith("  • ") || trimmed.startsWith("  → ")) {
      const bullet = trimmed.startsWith("  → ") ? "→" : trimmed.startsWith("  • ") ? "•" : "–";
      const text = trimmed.replace(/^[-•→]\s*/, "").replace(/^\s+[•→]\s*/, "");
      elements.push(
        <div key={i} className={`flex gap-2 text-sm py-0.5 ${trimmed.startsWith("  ") ? "pl-4" : ""}`}>
          <span className="text-muted-foreground flex-shrink-0">{bullet}</span>
          <span className="text-foreground/90">{renderInline(text)}</span>
        </div>
      );
    } else if (trimmed.startsWith("*Report generated")) {
      elements.push(<p key={i} className="text-xs text-muted-foreground mt-6 pt-3 border-t border-border/50 italic">{trimmed.replace(/^\*|\*$/g, "")}</p>);
    } else {
      elements.push(<p key={i} className="text-sm text-foreground/90 my-1">{renderInline(trimmed)}</p>);
    }
  }
...
export default function MatchAnalysisPage() {
  ...
  useEffect(() => {
    if (!matchId || tab !== "report" || report) return;
  ...
  if (error || !analysis) {
    return (
      <div className="space-y-4 pb-20 lg:pb-0">
        ...
        <Card><CardContent className="py-12 text-center text-red-400">{error || "Failed to load analysis"}</CardContent></Card>
        <MobileNav />
      </div>
    );
  }

  const { form, head_to_head: h2h, expected_goals: xg, goal_trends, league_position, patterns, match } = analysis;
  const home = analysis.home_team;
  const away = analysis.away_team;
  const probs = xg?.probabilities;
  ...
  return (
    <div className="space-y-6 pb-20 lg:pb-0">
      ...
      <Card className="border-primary/30">
        <CardContent className="py-6">
          <div className="text-center space-y-2">
            <p className="text-xs text-muted-foreground uppercase tracking-widest">{analysis.league}</p>
            <div className="flex items-center justify-center gap-4 sm:gap-8">
              <div className="text-right flex-1">
                <p className={`text-lg sm:text-xl font-bold ${match && match.home_score > match.away_score ? "text-green-400" : ""}`}>{home}</p>
              ...
              <div className="text-center flex-shrink-0">
                {match && match.home_score >= 0 ? (
                  <div className="text-3xl font-bold font-mono bg-secondary px-4 py-2 rounded-lg">
                    {match.home_score} - {match.away_score}
                  </div>
                ) : (
                  <div className="text-2xl font-bold text-muted-foreground">vs</div>
                )}
                {match?.home_score_ht != null && ((match.home_score_ht ?? 0) > 0 || (match.away_score_ht ?? 0) > 0) && (
                  <p className="text-xs text-muted-foreground mt-1">HT: {match.home_score_ht} - {match.away_score_ht}</p>
                )}
              </div>
              <div className="text-left flex-1">
                <p className={`text-lg sm:text-xl font-bold ${match && match.away_score > match.home_score ? "text-green-400" : ""}`}>{away}</p>
      ...
      {tab === "report" && (
        <Card>
          ...
          <CardContent>
            ...
            {reportError && (
              <p className="text-center text-red-400 py-8">{reportError}</p>
            )}
      ...
      {/* Match Stats (if finished match AND stats exist — hide when all zero) */}
      {match && match.home_score >= 0 && (
        ((match.home_shots ?? 0) > 0 || (match.away_shots ?? 0) > 0 || (match.home_possession != null && match.home_possession > 0) || (match.home_corners ?? 0) > 0 || (match.away_corners ?? 0) > 0)
      ) && (
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium flex items-center gap-2">
              <Target className="h-4 w-4 text-primary" /> Match Statistics
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="flex justify-between text-xs font-bold text-muted-foreground mb-2 px-2">
              <span>{home}</span><span>{away}</span>
            </div>
            <div className="space-y-0.5">
              {match.home_possession != null && <StatRow label="Possession" home={match.home_possession} away={match.away_possession} suffix="%" highlight />}
              <StatRow label="Shots" home={match.home_shots} away={match.away_shots} />
              <StatRow label="On Target" home={match.home_shots_on_target} away={match.away_shots_on_target} highlight />
              <StatRow label="Corners" home={match.home_corners} away={match.away_corners} />
              <StatRow label="Fouls" home={match.home_fouls} away={match.away_fouls} highlight />
              <StatRow label="Yellow Cards" home={match.home_yellow_cards} away={match.away_yellow_cards} />
              <StatRow label="Red Cards" home={match.home_red_cards} away={match.away_red_cards} highlight />
            </div>
          </CardContent>
        </Card>
      )}
      ...
      {/* Patterns & Anomalies */}
      {(patterns.home.length > 0 || patterns.away.length > 0) && (
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium flex items-center gap-2">
